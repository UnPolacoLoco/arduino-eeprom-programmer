...
void enableOutput() /* ensures write enable is disabled and output is enabled */
{
  digitalWrite(WRITE_EN, HIGH);
  int outputIO = 0;

  shiftOut(DATA_PIN, SR_CLK, LSBFIRST, outputIO);
  shiftOut(DATA_PIN, SR_CLK, LSBFIRST, (outputIO >> 8));
...
void showEEPROM()
{
  for (int address = 0; address <= dataLenght; address++)
  {
